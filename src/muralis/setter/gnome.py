...
class GnomeSetter(WallpaperSetter):
    ...
    def get_current_wallpaper(self) -> Optional[str]:
        ...
        Returns:
            Current wallpaper URI or None if not available
        """
        try:
            result = subprocess.run(
                ["gsettings", "get", self.SCHEMA, self.PICTURE_URI_KEY],
                capture_output=True,
                text=True,
                timeout=5
            )
            ...
            if result.returncode == 0:
                # Output format: "file:///path/to/image.jpg"
            ...
            logger.debug(f"Failed to get current wallpaper: {e}")
        
        return None
    
    def supports_da(self) -> bool:
        """Check if running under GNOME desktop environment.
        
        Returns:
            True if GNOME is detected, False otherwise
        """
        # Try gsettings command first
        if self._run_gsettings(self.PICTURE_URI_KEY, check=False):
            return True
        
        # Fallback to environment variable check
        import os
        desktop = os.environ.get('XDG_CURRENT_DESKTOP', '').lower()
        session = os.environ.get('GNOME_DESKTOP_SESSION_ID', '')
        
        return 'gnome' in desktop or bool(session)
    
    def get_gnome_version(self) -> Optional[tuple]:
        ...
        try:
            ...
            logger.debug(f"Failed to get GNOME version: {e}")
...
if __name__ == "__main__":
    ...
    setter = GnomeSetter()
    print(f"GNOME version: {setter.get_gnome_version()}")
    print(f"Dark mode support: {setter._check_dark_mode_support()}")
    print(f"GNOME detected: {setter.supports_da()}")
    
    # Test setting a wallpaper (uncomment to test)
    # setter.set_wallpaper("/path/to/your/wallpaper.jpg")